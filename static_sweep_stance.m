function Results = static_sweep_stance(sweepCfg)
    ...
    sweepCfg = fill_static_sweep_stance_defaults(sweepCfg, rootDir);
    ...
    matFile = fullfile(runDir, ['static_sweep_stance_results_' runStamp '.mat']);
    csvFile = fullfile(runDir, ['static_sweep_stance_cases_' runStamp '.csv']);
    tstCsvFile = fullfile(runDir, ['static_sweep_stance_summary_by_tst_' runStamp '.csv']);
    vaCsvFile = fullfile(runDir, ['static_sweep_stance_summary_by_va_' runStamp '.csv']);
    txtFile = fullfile(runDir, ['static_sweep_stance_log_' runStamp '.txt']);
    ...
    log_msg(fid, sweepCfg, 'STATIC SWEEP STANCE START');
    ...
    log_msg(fid, sweepCfg, 'Causality note: static stance sweep only diagnoses absolute combination feasibility from nominal reset. It cannot, by itself, prove abrupt-change failure.');
    log_msg(fid, sweepCfg, 'Tst_cmd is the commanded nominal stance-time parameter p.Tst. In trot, realized Tst inside fcn_FSM is speed-capped by min(p.Tst, 0.2 / norm(Xt(4:5))).');

    [gridTable, meta] = build_static_stance_grid(sweepCfg);
    nCases = height(gridTable);

    log_msg(fid, sweepCfg, sprintf('Tst count = %d', numel(sweepCfg.Tst_vals)));
    ...
    caseResults(nCases,1) = empty_case_result_stance();

    tSweep = tic;

    for idx = 1:nCases
        Tst_cmd = gridTable.Tst_cmd(idx);
        vCmd = gridTable.v_cmd(idx);
        aCmd = gridTable.a_cmd(idx);

        if sweepCfg.print_case_start
            log_msg(fid, sweepCfg, sprintf('START CASE %d/%d | Tst=%.3f | v=%.3f | a=%.3f', idx, nCases, Tst_cmd, vCmd, aCmd));
        end

        caseResult = simulate_static_stance_case(Tst_cmd, vCmd, aCmd, sweepCfg);
    ...
    caseTable = case_results_to_table_stance(caseResults);
    writetable(caseTable, csvFile);

    tstTable = summarize_by_tst(caseTable);
    writetable(tstTable, tstCsvFile);
    ...
    Results.summary_by_tst = tstTable;
    Results.summary_by_va = vaTable;
    Results.total_elapsed_sec = totalElapsed;

    save(matFile, 'Results');

    write_final_summary_stance(fid, sweepCfg, Results);

    log_msg(fid, sweepCfg, sprintf('MAT file: %s', matFile));
    log_msg(fid, sweepCfg, sprintf('Case CSV: %s', csvFile));
    log_msg(fid, sweepCfg, sprintf('Tst summary CSV: %s', tstCsvFile));
    log_msg(fid, sweepCfg, sprintf('VA summary CSV: %s', vaCsvFile));
    log_msg(fid, sweepCfg, 'STATIC SWEEP STANCE END');

    fprintf('[STATIC SWEEP STANCE END] run_dir=%s\n', runDir);
    fprintf('[STATIC SWEEP STANCE END] cases=%d, feasible=%d, infeasible=%d, elapsed=%.1f s\n', ...
        height(caseTable), sum(caseTable.feasible), sum(~caseTable.feasible), totalElapsed);
end

function sweepCfg = fill_static_sweep_stance_defaults(sweepCfg, rootDir)
    if ~isfield(sweepCfg, 'gait')
        sweepCfg.gait = 0;
    end

    if ~isfield(sweepCfg, 'chunk_duration')
        sweepCfg.chunk_duration = 1;
    end

    if ~isfield(sweepCfg, 'Tst_vals')
        sweepCfg.Tst_vals = 0.18:0.1:0.34;
    end

    if ~isfield(sweepCfg, 'v_vals')
        % First-pass range centered on the RL low-speed corridor and the
        % static/transition regions that were at least solver-reachable
        sweepCfg.v_vals = 0.6:0.1:1.1; % up to 1.1m/s
        % sweepCfg.v_vals = 0.3:0.1:0.8;
    end

    if ~isfield(sweepCfg, 'a_vals')
        % First-pass range kept moderate on purpose to avoid spending most
        % of the sweep in regions already showing strong failure tendency.
        % Broader second-pass option, if needed later:
        % sweepCfg.a_vals = 0.2:0.2:1.8;
        sweepCfg.a_vals = 0.2:0.2:1.2;
    end
...
function [gridTable, meta] = build_static_stance_grid(sweepCfg)
    [TST, VV, AA] = ndgrid(sweepCfg.Tst_vals, sweepCfg.v_vals, sweepCfg.a_vals);
    ...
    if sweepCfg.use_feasible_va_mask
        ...
        mask = true(size(tstList));
    end

    gridTable = table(tstList(mask), vList(mask), aList(mask), ...
        'VariableNames', {'Tst_cmd', 'v_cmd', 'a_cmd'});

    gridTable = sortrows(gridTable, {'v_cmd', 'a_cmd', 'Tst_cmd'});

    meta = struct();
    meta.Tst_vals = sweepCfg.Tst_vals;
    ...
    meta.static_scope = 'absolute_Tst_v_a_combination_from_nominal_reset_only';
    meta.realized_Tst_note = 'realized Tst in trot is speed-capped inside fcn_FSM';
end
...
function caseResult = simulate_static_stance_case(Tst_cmd, vCmd, aCmd, sweepCfg)
    ...
    expectedTstCap = 0.2 / max(vCmd, 1e-9);

    caseResult = empty_case_result_stance();
    caseResult.Tst_cmd = Tst_cmd;
    ...
    caseResult.nominal_Tst = nominalTst;
    caseResult.expected_Tst_cap_vcmd = expectedTstCap;
    caseResult.expected_cap_limited = Tst_cmd > expectedTstCap + 1e-9;
    ...
    caseResult.causality_scope = 'static_stance_combination_only';
    ...
    if isempty(caseResult.fail_reason)
        ...
        caseResult.root_cause_inference = 'static_Tst_v_a_combination_feasible_from_nominal_reset';
    end
...
function T = case_results_to_table_stance(caseResults)
    n = numel(caseResults);
    ...
    feasible = false(n,1);
    nominal_Tst = nan(n,1);
    expected_Tst_cap_vcmd = nan(n,1);
    expected_cap_limited = false(n,1);
    ...
    for i = 1:n
        ...
        nominal_Tst(i) = c.nominal_Tst;
        expected_Tst_cap_vcmd(i) = c.expected_Tst_cap_vcmd;
        expected_cap_limited(i) = logical(c.expected_cap_limited);
    ...
    T = table( ...
        case_idx, Tst_cmd, v_cmd, a_cmd, feasible, nominal_Tst, expected_Tst_cap_vcmd, expected_cap_limited, fail_reason, root_cause_inference, ...
...
function T = summarize_by_tst(caseTable)
    tstVals = unique(caseTable.Tst_cmd);
    n = numel(tstVals);
    ...
    mean_realized_Tstance = nan(n,1);
    mean_realized_Tcycle = nan(n,1);
    cap_limited_ratio = nan(n,1);
    mean_fail_time = nan(n,1);

    for i = 1:n
        tst = tstVals(i);
        mask = caseTable.Tst_cmd == tst;
        ...
        mean_realized_Tstance(i) = safe_mean(caseTable.Tstance_mean(mask & caseTable.feasible));
        mean_realized_Tcycle(i) = safe_mean(caseTable.Tcycle_mean(mask & caseTable.feasible));
        cap_limited_ratio(i) = mean(double(caseTable.expected_cap_limited(mask)));
        mean_fail_time(i) = safe_mean(caseTable.fail_time_s(mask & ~caseTable.feasible));
    end

    T = table(Tst_cmd, total_cases, feasible_cases, infeasible_cases, feasible_ratio, ...
        mean_tracking_error, mean_control_effort, mean_Ieq, mean_soc_end, ...
        mean_realized_Tstance, mean_realized_Tcycle, cap_limited_ratio, mean_fail_time);
end
...
function T = summarize_by_va(caseTable)
    ...
    feasible_cases = zeros(n,1);
    infeasible_cases = zeros(n,1);
    max_feasible_Tst = nan(n,1);
    min_infeasible_Tst = nan(n,1);
    ...
    for i = 1:n
        mask = G == i;
        Tsub = sortrows(caseTable(mask,:), 'Tst_cmd');
        ...
        tstFeas = Tsub.Tst_cmd(Tsub.feasible);
        tstInfeas = Tsub.Tst_cmd(~Tsub.feasible);

        if ~isempty(tstFeas)
            max_feasible_Tst(i) = max(tstFeas);
        end
        if ~isempty(tstInfeas)
            min_infeasible_Tst(i) = min(tstInfeas);
        end

        threshold_detected(i) = ~isempty(tstFeas) && ~isempty(tstInfeas) && max(tstFeas) < min(tstInfeas);
        mean_realized_Tstance(i) = safe_mean(Tsub.Tstance_mean(Tsub.feasible));
        mean_fail_time(i) = safe_mean(Tsub.fail_time_s(~Tsub.feasible));
    end

    T = table(v_cmd, a_cmd, total_cases, feasible_cases, infeasible_cases, ...
        max_feasible_Tst, min_infeasible_Tst, threshold_detected, mean_realized_Tstance, mean_fail_time);
end

function c = empty_case_result_stance()
    c = struct( ...
        'case_idx', NaN, ...
        'run_stamp', '', ...
        'run_dir', '', ...
        ...
        'max_iter', NaN, ...
        'nominal_Tst', NaN, ...
        'expected_Tst_cap_vcmd', NaN, ...
        'expected_cap_limited', false, ...
        'feasible', false, ...
        'fail_reason', '', ...
        'fail_reason_detail', '', ...
        'fail_iter', NaN, ...
...
function s = format_case_log_line(caseResult, nCases)
    if caseResult.feasible
        s = sprintf(['CASE %d/%d | Tst=%.3f | v=%.3f | a=%.3f | feasible=1 | ' ...
            'Tst_mean=%.3f | track=%.3f | effort=%.3f | Ieq=%.3f | SOC_end=%.2f | elapsed=%.2f s'], ...
            caseResult.case_idx, nCases, caseResult.Tst_cmd, caseResult.v_cmd, caseResult.a_cmd, ...
            caseResult.Tstance_mean, caseResult.tracking_error_mean, caseResult.control_effort_mean, ...
            caseResult.Ieq_A, caseResult.soc_end_pct, caseResult.elapsed_sec);
    else
        s = sprintf(['CASE %d/%d | Tst=%.3f | v=%.3f | a=%.3f | feasible=0 | ' ...
...
function write_final_summary_stance(fid, sweepCfg, Results)
    ...
    log_msg(fid, sweepCfg, sprintf('Expected cap-limited cases = %d', sum(T.expected_cap_limited)));
    if feasibleCount > 0
        log_msg(fid, sweepCfg, sprintf('Feasible mean realized Tstance = %.4f s', safe_mean(T.Tstance_mean(T.feasible))));
