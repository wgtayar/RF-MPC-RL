function report = run_phase3_environment_validation(outputRoot, shortProbe)
...
% 10-step decisions plus MATLAB validateEnvironment's captured 10-step probe.
    ...
    env = Phase3MpcEnvironment(bundle,outputRoot,metadata, ...
        struct('reference_mode','position_continuous_v1','solver_strategy','default'));
    ...
    for decision = 1:2
        ...
        assert(isequal(size(observation),[19,1]) && all(isfinite(observation)) && isfinite(reward), ...
    ...
    report = struct('source_sha',env.Writer.Manifest.source_sha, ...
        'source_dirty',env.Writer.Manifest.source_dirty,'short_probe',shortProbe, ...
