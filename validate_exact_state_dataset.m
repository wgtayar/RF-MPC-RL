...
%validate_exact_state_dataset Validate hashes, provenance and MPC continuity.
    ...
    for k = 1:height(index)
        ...
        data = load(target,'rows');
        assert(numel(data.rows) == index.row_count(k) && ...
            data.rows{1}.row_id == index.first_row(k) && ...
            data.rows{end}.row_id == index.last_row(k), ...
        ...
        for j = 1:numel(data.rows)
            ...
            localReference(root,row.snapshot_reference);
            if strlength(string(row.qp_reference.file)) > 0
                target = localReference(root,row.qp_reference);
...
function target = localReference(root, reference)
    target = localArtifact(root,reference.file);
    assert(strcmp(sha256_file(target),reference.sha256), ...
